...
export interface AportesSocioData {
  organizacion: string
  lema: string
  fechaEmision: string
...
.tabla th.monto { text-align: right; }
...
function filaAporte(item: AporteItem): string {
  const estadoClase = item.estado === 'ANULADO' ? 'estado-anulado' : 'estado-activo'
  return `
    <tr>
      <td>${esc(item.tipo)}</td>
      <td>${esc(item.periodo)}</td>
      <td>${esc(item.fecha)}</td>
      <td class="monto">${esc(item.monto)}</td>
      <td><span class="${estadoClase}">${esc(item.estado === 'ANULADO' ? 'Anulado' : 'Vigente')}</span></td>
    </tr>`
}

export function buildAportesSocioHtml(data: AportesSocioData): string {
  const gruposHtml = data.grupos.map((grupo) => `
    <section class="grupo">
      ...
      <table class="tabla">
        ...
        <tbody>
          ${grupo.items.map(filaAporte).join('\n')}
        </tbody>
      </table>
    </section>`).join('\n')

  const cuerpo = data.grupos.length === 0
    ? `<p class="sin-datos">El socio no registra aportes</p>`
    : `${gruposHtml}
      <div class="resumen">
        <div class="caja-total">
          <p class="total-etiqueta">Total Aportado</p>
...
<head>
  <meta charset="UTF-8" />
  <title>Historial de Aportes del Socio</title>
  <style>${css}</style>
</head>
<body>
  <div class="hoja">
    ...
    <header class="cabecera">
      ...
      <div class="membrete">
        ...
        <h1>Historial de Aportes del Socio</h1>
      </div>
